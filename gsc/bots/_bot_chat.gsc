...
init()
{
	...
	// One shared chat budget for the whole match.
	// randomint( 5 ) returns 0-4, giving us 2-6 total messages.
	level.bot_chat_limit = randomint( 5 ) + 2;
	level.bot_chat_count = 0;
	level.bot_chat_next_time = gettime() + 30000;
...
BotDoChat( chance, string, isTeam )
{
	...
	// Hard match-wide limit: 2-6 messages total.
	if ( level.bot_chat_count >= level.bot_chat_limit )
	{
		return;
	}
	
	// Keep at least 35-75 seconds between messages from any bot.
	if ( gettime() < level.bot_chat_next_time )
	{
		return;
	}
	...
	{
		level.bot_chat_count++;
		level.bot_chat_next_time = gettime() + 35000 + randomint( 40000 );
...
start_chat_threads()
{
	...
	// Keep chat sparse and lobby-like instead of reacting to every event.
	self thread start_realistic_chat();
}
...
/*
	Sparse, realistic public-lobby chatter.
	The global limit and cooldown are enforced inside BotDoChat().
*/
start_realistic_chat()
{
	self endon( "disconnect" );
	level endon( "game_ended" );
	
	// Do not let bots start talking immediately when the match loads.
	wait ( 35 + randomint( 45 ) );
	
	for ( ;; )
	{
		if ( isreallyalive( self ) )
		{
			self BotDoChat( 35, get_realistic_chat_message() );
		}
		
		// Each bot only attempts occasionally. The global cooldown prevents pileups.
		wait ( 35 + randomint( 70 ) );
	}
}
...
get_realistic_chat_message()
{
	switch ( randomint( 24 ) )
	{
		...
		case 6:  return "okay dude";
		case 7:  return "of course";
		case 8:  return "this team man";
		case 9:  return "where is my team";
		case 10: return "stop camping";
		case 11: return "nice spawn";
		case 12: return "bro";
		case 13: return "no way";
		case 14: return "that timing";
		case 15: return "who is using assassin";
		case 16: return "can we cap";
		case 17: return "play the objective";
		case 18: return "stuns are so dumb";
		case 19: return "another corner camper";
